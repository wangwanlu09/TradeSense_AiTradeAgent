...
export function StockNews() {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            try {
                ...
                // 尝试使用直接方法获取新闻数据
                const data = await apiService.directData.getBusinessNews();
    ...
    if (loading) return <div className="p-4 text-gray-300 bg-black h-screen">Loading business news...</div>;
    if (error) return <div className="p-4 text-red-500 bg-black h-screen">Error: {error}</div>;
    if (articles.length === 0) return <div className="p-4 text-gray-300 bg-black h-screen">No business news available</div>;
